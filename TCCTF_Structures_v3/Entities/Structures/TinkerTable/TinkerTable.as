...
void onInit(CBlob@ this)
{
	...
	{
		ShopItem@ s = addShopItem(this, "Autonomous Activator", "$icon_automat$", "automat", "A contraption that uses items automatically while targetting enemies. May be only carried around when not holding anything.");
...
void onTick(CBlob@ this)
{
	...
	if (getMap().getBlobsInBox(this.getPosition() + Vec2f(96, 64), this.getPosition() + Vec2f(-96, 0), @blobs))
	{
		for (uint i = 0; i < blobs.length; i++)
		{
			...
			{
				if (blob.getHealth() < blob.getInitialHealth()) blob.server_Heal(10);
			}
		}
	}
}

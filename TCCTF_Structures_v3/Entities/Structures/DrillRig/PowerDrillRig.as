...
void onTick(CBlob@ this)
{
	if (isServer())
	{
		...
		if (this.get_bool("bedrock_drillrig"))
		{
			...
			MakeMat(this, drillPos, resources[index], (1 + XORRandom(resourceYields[index]))*(this.get_f32("gyro_bonus")*2 + this.get_u8("drill_count")));
		}
		else
		{
			CMap@ map = getMap();
			...
			for (u8 i = 0; i<this.get_u8("drill_count");i++)
			{
				...
				Vec2f pos = Vec2f(drillPos.x + (XORRandom(48) - 24) * (1 - depth / 96), Maths::Min(drillPos.y + 16 + depth, (map.tilemapheight * map.tilesize) - 8));

				this.server_HitMap(pos, Vec2f(0, 0), 1.3f, Hitters::drill);
			}
			this.add_u8("spam", 1);
			if (this.get_u8("spam") > 225)
			{
				this.set_u8("spam", 0);
				CBitStream params;

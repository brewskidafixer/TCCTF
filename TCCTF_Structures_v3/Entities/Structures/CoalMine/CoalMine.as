...
void onInit(CBlob@ this)
{
	...
	for (u8 i=0;i<2;i++)
	{
		...
		if (i == 1) bulk = 2500;
		{
			ShopItem@ s = addShopItem(this, "Buy Dirt ("+bulk+")", "$mat_dirt$", "mat_dirt-"+bulk, "Buy "+bulk+" Dirt for "+bulk/2+" coins.");
			AddRequirement(s.requirements, "coin", "", "Coins", bulk/2);
			s.spawnNothing = true;
		}
		{
		...
		{
			ShopItem@ s = addShopItem(this, "Buy Sulphur ("+bulk/5+")", "$mat_sulphur$", "mat_sulphur-"+bulk/5, "Buy "+bulk/5+" Sulphur for "+bulk+" coins.");
			AddRequirement(s.requirements, "coin", "", "Coins", bulk);
			s.spawnNothing = true;
		}
	}
...
{
	return (forBlob !is null && this.getDistanceTo(forBlob) <= 64);
}
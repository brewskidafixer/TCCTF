...
{
	if (blob is null || blob.hasTag("player") || blob.hasTag("human")) return;
	if (blob.get_u8("filter_move") > 15) return;
	...
	if(server_isItemAccepted(this, blob.getName())){
		...
		if (Maths::Abs(blob.getVelocity().y) < 2.0f){
			blob.setVelocity(Vec2f(this.isFacingLeft() ? -1.0f : 1.0f, -1.0f));
			blob.add_u8("filter_move", 1);
		}
	}
}

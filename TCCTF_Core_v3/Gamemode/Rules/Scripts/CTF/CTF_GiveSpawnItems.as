...
const u32 materials_wait_warmup = 80; //seconds between free mats
...
bool GiveSpawnResources(CRules@ this, CBlob@ blob, CPlayer@ player, CTFPlayerInfo@ info)
{
	...
	if (blob.getName() == "builder" || blob.getName() == "engineer")
	{
		if (this.isWarmup())
		{
			ret = SetMaterials(blob, "mat_wood", 1000) || ret;
			ret = SetMaterials(blob, "mat_stone", 750) || ret;
			ret = SetMaterials(blob, "mat_concrete", 250) || ret;
			ret = SetMaterials(blob, "mat_ironingot", 40) || ret;
			ret = SetMaterials(blob, "mat_copperingot", 10) || ret;
			ret = SetMaterials(blob, "mat_steelingot", 5) || ret;
		}
		else
		{
			ret = SetMaterials(blob, "mat_wood", 250) || ret;
			ret = SetMaterials(blob, "mat_concrete", 25) || ret;
			ret = SetMaterials(blob, "mat_ironingot", 5) || ret;
...
void onTick(CRules@ this)
{
	...
	if (core !is null)
	{
		...
		getBlobsByName("tinkertable", @spots);
		...
		for (uint step = 0; step < spots.length; ++step)
		{
			...
			if (spot !is null && spot.getOverlapping(overlapping))
			{
				...
				for (uint o_step = 0; o_step < overlapping.length; ++o_step)
				{
					...
					{
						if (!isShop || overlapped.getName() == "builder" ||  overlapped.getName() == "engineer")
						{

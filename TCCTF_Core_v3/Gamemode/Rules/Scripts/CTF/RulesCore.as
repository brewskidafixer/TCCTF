...
shared class RulesCore
{
	...
	void ChangePlayerTeam(CPlayer@ player, int newTeamNum)
	{
		...
		//if(is_spawning || oldteam == rules.getSpectatorTeamNum())
		{
			...
			if (oldBlob !is null && !oldBlob.hasTag(player.getUsername())) oldBlob.Untag(player.getUsername());
	...
	void AddPlayerSpawn(CPlayer@ player)
	{
		...
		else
		{
			...
			// kill old player
			CBlob@ oldBlob = player.getBlob();
			if (oldBlob !is null && !oldBlob.hasTag(player.getUsername())) RemovePlayerBlob(player);
		}

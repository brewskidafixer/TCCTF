	...
	if (cmd == this.getCommandID("equip_head") || cmd == this.getCommandID("equip_torso") || cmd == this.getCommandID("equip_boots"))
	{
		...
		if (item !is null)
		{
			...
			if (getEquipmentType(item) == "head")
			{
				...
				if (eqName == "lighthelmet" || eqName == "mediumhelmet" || eqName == "heavyhelmet" || eqName == "bucket" || eqName == "pumpkin" || 
				...
				if (item.hasTag("nightGoggles")) caller.Tag("nightGoggles");
			...
			else if (getEquipmentType(item) == "torso")
			{
				...
				if (eqName == "lightvest" || eqName == "mediumvest" || eqName == "heavyvest" || eqName == "keg") caller.set_f32(eqName+"_health", item.get_f32("health"));
				if (item.getQuantity() <= 1) item.server_Die();
				else item.server_SetQuantity(Maths::Max(item.getQuantity() - 1, 0));
			}
...
void removeHead(CBlob@ playerblob, string headname)
{
	...
	if (playerblob.getSprite().getSpriteLayer("nightGoggles") !is null)
	{
		playerblob.getSprite().RemoveSpriteLayer("nightGoggles");
		if (isClient() && playerblob.isMyPlayer()) getMap().CreateSkyGradient("skygradient.png");
	}
	if (headname == "minershelmet") playerblob.SetLight(false);	//example of removing custom tags like Light
	if (headname == "heavyhelmet")
	{
	...
	if (isServer())
	{
		...
		if (headname == "lighthelmet" || headname == "mediumhelmet")
		{
			if (playerblob.hasTag("bushy")) oldeq.Tag("bushy");
			if (playerblob.hasTag("nightGoggles")) oldeq.Tag("nightGoggles");
			oldeq.set_f32("health", playerblob.get_f32(headname+"_health"));
		}
		else if (headname == "heavyhelmet" || headname == "bucket" || headname == "pumpkin" || headname == "scubagear" || headname == "minershelmet")
		{
			oldeq.set_f32("health", playerblob.get_f32(headname+"_health"));
		}
	...
	if (playerblob.hasTag("nightGoggles")) playerblob.Untag("nightGoggles");
}
...
void onDie(CBlob@ this)
{
    if (isServer())
	{
		...
		if (headname != "")
		{
			...
			else if (headname != "lighthelmet" && headname != "mediumhelmet" && headname != "heavyhelmet") server_CreateBlob(headname, this.getTeamNum(), this.getPosition());
		}
		if (torsoname != "")
		{
			if (torsoname == "lightvest" || torsoname == "mediumvest" || torsoname == "heavyvest")
			{

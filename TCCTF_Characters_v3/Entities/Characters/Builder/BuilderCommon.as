		...
		if (b.buildOnGround)
		{
			...
			if (!fail)
			{
				...
				if (!fail && b.name == "camp")
				{
					...
					u8 teamCount = 0; // counts team bases
					//getBlobsByTag("upf_base", @blobs);
					bool isMe = this.isMyPlayer();
					for (int i = 0; i < blobs.length; i++)
					{
						CBlob@ e = blobs[i];
						Vec2f vector = e.getPosition() - pos;
						f32 distance = vector.getLength();
						bool isMyTeam = e.getTeamNum() == this.getTeamNum();
						if (isMyTeam) teamCount++;
						if (blobs.length >= 12)
						{
							fail = true;
							if (isMe) client_AddToChat("Global Faction Base Limit reached!", SColor(0xff444444));
							break;
						}
						else if (e.hasTag("faction_base") && !isMyTeam && distance <= 400 && distance >= 8)
						{
							fail = true;
							if (isMe) client_AddToChat("There is an enemy faction base too close!", SColor(0xff444444));
							break;
						}
						else if (e.hasTag("faction_base") && distance <= 64 && distance >= 8)
						{
							fail = true;
							if (isMe) client_AddToChat("There is a faction base too close!", SColor(0xff444444));
							break;
						}
						//else if (e.hasTag("upf_base") && distance <= 256 && distance >= 8)
						//{
						//	fail = true;
						//	if (isMe) client_AddToChat("There is a UPF base too close!", SColor(0xff444444));
						//	break;
						//}
					}
					if (!fail)
					{
						...
						{
							fail = true;
							if (isMe) client_AddToChat("Team Base Limit reached!", SColor(0xff444444));
						}
					}
				}
			}

...
void onTick(CSprite@ this)
{
	...
	else
	{
		...
		{
			blob.set_u32("next meow", getGameTime() + 400);
			this.PlaySound("Kitten_Meow_" + XORRandom(5), 0.20f, 1);
		}
	}
}
...
void onInit(CBlob@ this)
{
	...
	if (!this.exists("voice_pitch")) this.set_f32("voice pitch", 1.0f);
}
...
void onTick(CBlob@ this)
{
	if (!this.hasTag("dead"))
	{
		...
		if (this.getHealth() < 0)
		{
			...
			this.getSprite().PlaySound("Kitten_Death_0", 0.15f, 1.0f);
			this.Tag("dead");
			// this.getCurrentScript().removeIfTag = "dead";
		}
			...
			if (inventoryBlob !is null)
			{
				if (this.getTickSinceCreated() % 10 == 0)
				{
					...
					if (isClient()) 
					{
						if (XORRandom(3) == 0) 
						{	
							this.getSprite().PlaySound("Kitten_Hit_" + XORRandom(4), 0.30f, 1.0f);
							this.set_u32("next screech", getGameTime() + 40);
						}
						if (inventoryBlob.hasTag("flesh"))
						{
							this.getSprite().PlaySound("Pus_Attack_" + XORRandom(3), 0.3f, 1.00f);
...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	{
		this.getSprite().PlaySound("Kitten_Hit_" + XORRandom(4), 0.10f, 1.0f);
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	...
	if (blob.getName() == "mat_mithril" && blob.getQuantity() > 200)
	{
		if (isServer())
		{

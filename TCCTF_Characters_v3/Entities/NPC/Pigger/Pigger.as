...
void onInit(CBlob@ this)
{
	...
	this.set_f32("pickup_priority", 0.02f);
}
		...
		if (this.hasTag("dead"))
		{
			...
			if (this.getOverlapping(blobs))
			{
				for (u8 i = 0; i < blobs.length; i++)
				{
					...
					{
						CBlob@ other = blobs[i];
						if (other !is null && other.getName() == "pigger")
						{
							u8 other_quantity = other.getQuantity();
							if (other_quantity < 64)
							{
			...
			CBlob@ mithril = server_CreateBlob("mat_mithril", this.getTeamNum(), this.getPosition());
			mithril.server_SetQuantity((3 + XORRandom(10))*this.getQuantity());
		}
		else if (XORRandom(500) == 0)
		{
			CBlob@ mithril = server_CreateBlob("mat_mithril", this.getTeamNum(), this.getPosition());
			mithril.server_SetQuantity(2 + XORRandom(10));
			this.server_Hit(this, this.getPosition(), Vec2f(0, 0), 1.0f, Hitters::stab, true);
		}
	}
}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid)
{
	...
	if (blob !is null && getGameTime() >= this.get_u32("next attach") && this.get_u16("succ netid") == 0 && blob.hasTag("human") && !blob.hasTag("dead") && !blob.hasTag("pigger immune"))
	{

				...
				if (getMap().getHitInfosFromArc(this.getPosition(), -(aimDir).Angle(), 35, maxDistance, this, @hitInfos))
				{
					for (uint i = 0; i < hitInfos.length; i++)
					{
						...
						if (blob !is null)
						{
							...
							{
								if (blob.getTeamNum() != holder.getTeamNum()) blob.AddForce(dir * Maths::Min(50, blob.getMass()) * ((maxDistance - dist) / maxDistance * 0.80f));
							}
							else blob.AddForce(dir * Maths::Min(50, blob.getMass()) * ((maxDistance - dist) / maxDistance * 0.80f));

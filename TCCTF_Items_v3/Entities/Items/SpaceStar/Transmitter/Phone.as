		...
		if (isServer())
		{
			...
			else
			{
				...
				else if (name == "minefield")
				{
					for (int i = 0; i < 10; i++)
					{
						CBlob@ blob = server_MakeCrateOnParachute("mine", "SpaceStar Ordering Mines", 0, callerBlob.getTeamNum(), Vec2f(callerBlob.getPosition().x + (256 - XORRandom(512)), XORRandom(64)));
						blob.Tag("unpack on land");
						blob.Tag("destroy on touch");
					}
				}

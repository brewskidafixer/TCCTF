			...
			if (lmb && this.get_u32("nextShoot") <= getGameTime() && HasAmmo(holder, true, this.get_string("ammoBlob")))
			{
				...
				if (getMap().getHitInfosFromRay(startPos, angle + (flip ? 180 : 0), maxDistance, holder, blobs))
				{
					for (int i = 0; i < blobs.length; i++)
					{
						...
						f32 factor = (maxDistance - blobs[i].distance)/maxDistance;

						if (isServer())
						{
							this.server_Hit(b, b.getPosition(), Vec2f(0, 0), maxDamage*factor, HittersTC::electric, true);
						}

						SetKnocked(b, 60*factor);

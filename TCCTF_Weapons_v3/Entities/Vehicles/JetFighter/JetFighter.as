...
void onTick(CBlob@ this)
{
	...
	if (isClient())
	{
		this.getSprite().SetEmitSoundSpeed(0.15f + (this.get_f32("velocity") / SPEED_MAX * 0.2f) * (this.getVelocity().Length() * 0.15f));
...
void onCollision(CBlob@ this,CBlob@ blob,bool solid)
{
	...
	if (power > 7.0f && blob == null)
	{
		...
		this.server_Hit(this, this.getPosition(), Vec2f(0, 0), (this.getAttachments().getAttachmentPointByName("FLYER") is null || this.getHealth() < 2.0f) ? power * 1.5f : power * 0.001f, 0, true);
	}
}
...
void onAttach(CBlob@ this, CBlob@ attached, AttachmentPoint @attachedPoint)
{
	if (attached !is null && attached.hasTag("player"))
	{
		...
		CSpriteLayer@ milhelmet = attached.getSprite().getSpriteLayer("militaryhelmet");
...
void onDetach(CBlob@ this, CBlob@ detached, AttachmentPoint @attachedPoint)
{
	if (detached !is null && detached.hasTag("player"))
	{
		...
		CSpriteLayer@ milhelmet = detached.getSprite().getSpriteLayer("militaryhelmet");
		if (milhelmet !is null)
		{
...
void onRender(CSprite@ this)
{
	...
	if (fuel <= 0 && mouseOnBlob)
	{
		...
		GUI::SetFont("menu");
		GUI::DrawTextCentered("This vehicle requires fuel to fly!", Vec2f(pos.x, pos.y + 100 + Maths::Sin(getGameTime() / 5.0f) * 5.0f), SColor(255, 255, 55, 55));
...
void renderAmmo(CBlob@ blob, bool rocket)
{
	...
	f32 dist = lr.x - ul.x;
	Vec2f upperleft((ul.x + (dist / 2.0f)) - 5.0f + 4.0f, pos2d1.y + blob.getHeight() + 60);
	Vec2f lowerright((ul.x + (dist / 2.0f))  + 5.0f + 4.0f, upperleft.y + 20);

	//GUI::DrawRectangle(upperleft - Vec2f(0,20), lowerright , SColor(255,0,0,255));

	u16 ammo = GetAmmo(blob);

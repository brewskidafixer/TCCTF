...
void onTick(CBlob@ this)
{
	...
	if (this.getHealth() <= 0.0f || this.isKeyJustPressed(key_action3))
	{
		...
		if (payload !is null)
		{
			...
			else if (payload.getName() == "mat_dynamite")
			{
			...
			else if (payload.getName() == "mat_fraggrenade")
			{
				if(isServer())
				{
					CBlob@ dynamite = server_CreateBlob("fraggrenade", this.getTeamNum(), this.getPosition());
					payload.server_Die();
				}
			}
			else if (payload.getName() == "fragmine")
			{
	...
	if (this.hasTag("offblast"))
	{
		...
		if (this.get_u32("fuel_timer") > getGameTime())
		{
			...
			this.set_f32("velocity", Maths::Min(this.get_f32("velocity") + 0.1f, this.get_f32("max_velocity")));

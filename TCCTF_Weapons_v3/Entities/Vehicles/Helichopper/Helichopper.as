...
const Vec2f minClampVelocity = Vec2f(-0.20f, -0.70f);
const Vec2f maxClampVelocity = Vec2f( 0.20f, 0.00f);
...
void onInit(CBlob@ this)
{
	...
	settings.B_DAMAGE = 3.5f; //Bullet damage
...
void onTick(CBlob@ this)
{
	if (this !is null)
	{
		...
		if (fuel <= 0) targetForce = Vec2f(0,0);
...
void GetButtonsFor(CBlob@ this, CBlob@ caller)
{
	if (caller.getTeamNum() == this.getTeamNum())
	{
		...
		if (ammoCount > 0)
		{
			CBitStream params;
			params.write_u16(caller.getNetworkID());
			caller.CreateGenericButton(20, Vec2f(17, 8), this, 
				this.getCommandID("takeAmmo"), getTranslatedString("Take out Gatling Ammo"), params);
		}
		if (ammoCount < maxAmmo)
		{
			...
			CBitStream params;
			params.write_u16(caller.getNetworkID());
			caller.CreateGenericButton("$icon_gatlingammo$", Vec2f(17, 2), this, 
				this.getCommandID("addAmmo"), getTranslatedString("Insert Gatling ammo"), params);
		}
		{
			CBitStream params;
...
void shootRocket(CBlob@ this, f32 angle, CBlob@ target, Vec2f gunPos)
{
	...
	if (isClient())
	{
		for (int i = 1; i < 3; i++) {MakeParticle(this, -dir * i, "SmallExplosion");}
